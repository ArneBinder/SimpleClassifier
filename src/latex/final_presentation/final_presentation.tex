...
%frame elements mit dem bezug auf ein (im Model) bekanntes target. counts erklaeren und ausgeben 
	...
	\frame{
		...
		\begin{figure}
			...
			\caption{Ergebnisse der 5-fach Kreuzvalidierung}
		\end{figure}
...
% 		\end{flushright}
	...
	\frame{
		...
		\caption{Closed Challenge, SRL-only Task, Semantic Labeled F1 \cite{hajivc2009conll}}
		\end{table}
	}
	...
	\frame{
		...
		\begin{itemize}[label=$\bullet$,itemsep=1.5ex]
			...
			%\vspace{1cm}
			\item SHALMANESER \cite{erk2006shalmaneser}
				% Universität Saarland
			\begin{itemize}[label=$-$,itemsep=1ex]
			  \item Grundlage: SALSA 1.0 \cite{burchardt2006salsa} 
				\item Prec.: 0.761  $\vert$ Recall: 0.496  $\vert$ F-Measure: 0.600
			...
			\item Bedeutenste Features: Pfad \& Nachbar-Kopf
