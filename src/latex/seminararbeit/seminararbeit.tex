...
Da die Frame-Elemente als relevante Informationsträger angesehen werden, wurde analog zu \cite{gildea} die Identifikation des durch einen Satz beziehungsweise eine Phrase realisierten Frames sehr vereinfacht. Es wird die Annahme verfolgt, dass gleiche Rollen auch in verschiedenen Frames ähnlich morphosyntaktisch realisiert werden. So lassen sich die nur sehr spärlich zur Verfügung stehenden Trainingsdaten\footnote{Für den Gesamtkorpus gilt: es gibt pro FE durchschnittlich 100 Instanzen} besser ausschöpfen. Einzig über das Prädikat werden implizit künstlich Superframes[TODO: oder Hyper-...?] erzeugt, die alle Frames mit dem gleichen Target zusammenfassen.

Um die Hypothese zu verifizieren wurde ein Naive-Bayes-Klassifikator implementiert. Dieses statistische Lernverfahren wurde gewählt, da es im Vergleich zu anderen relativ transparent ist. Die dem trainierten Modell inhärenten Wahrscheinlichkeitsverteilungen liefern meist auch vom Menschen leicht interpretierbare Informationen, welche bei der Entwicklung des Klassifizierers sehr hilfreich waren. 

Da allerdings einige Beziehungen zwischen den einzelnen Features semantisch relevant sind 

 Auf Grund der geringen Trainingsdatenmenge würde es sehr häufig vorkommen, dass eine konkrete Kombination von Features beim Klassifizieren    
Back-off
...
- keine Frames über mehrere Sätze, keine Bestimmung der Frames an sich, keine Abstraktion für ungesehene Targets 
\subsection{Features}
